...
import { SheetRow } from '@/app/lib/interfaces';
...
describe('GET /api/download-excel', () => {
    ...
    it('should fetch data from Google Sheets', async () => {
        ...
        (googleSheets.getSheetData as jest.MockedFunction<typeof googleSheets.getSheetData>).mockResolvedValue(mockSheetData);
        (excelService.ExcelService.downloadExcel as jest.MockedFunction<typeof excelService.ExcelService.downloadExcel>).mockResolvedValue(new NextResponse(Buffer.from('test'), {
            headers: new Headers({
        ...
        const response = await GET(
            new Request('http://localhost/api/download-excel?folder_id=1')
        );
    ...
    it('should filter by status_id=7', async () => {
        ...
        (googleSheets.getSheetData as jest.Mock).mockResolvedValue(mockSheetData);

        await GET(new Request('http://localhost/api/download-excel?folder_id=1'));

        const filteredData = (excelService.ExcelService.downloadExcel as jest.Mock).mock.calls[0][1];
    ...
    it('should group by folder_id', async () => {
        ...
        const excelData = (excelService.ExcelService.downloadExcel as jest.Mock).mock.calls[0][1];
