  ...
  try {
    ...
    if (scenarioId === 'icpu_check') {
      ...
      return NextResponse.json({ message: `Email for scenario '${scenarioId}' sent successfully with ${dataToSend.length} items.` }, { status: 200 });
        ...
        // Check which positions have already been sent
        const sentIds = EmailLogger.getSentRequestIds('category_manager_notification', 7); // 7 days back
        const newPositions = filteredPositions.filter(pos => 
            !sentIds.includes(pos.request_position_id)
        );
        ...
        // Check if we have email configuration for this category
        const categoryConfig = (scenario as EmailScenario & { category_mapping?: Record<string, { to: string[]; cc: string[]; bcc: string[] }> }).category_mapping?.[categoryId.toString()];
        if (!categoryConfig) {
            return NextResponse.json({ 
                error: `No email configuration found for category ${categoryId}` 
            }, { status: 400 });
        }
        ...
        const toRecipients = categoryConfig.to.filter((email: string) => email && email.trim() !== '');
        const ccRecipients = categoryConfig.cc.filter((email: string) => email && email.trim() !== '');
        const bccRecipients = categoryConfig.bcc.filter((email: string) => email && email.trim() !== '');

        if (toRecipients.length === 0) {
            return NextResponse.json({ error: 'No valid recipients found for this category' }, { status: 400 });
        }
