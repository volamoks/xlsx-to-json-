  ...
  try {
    ...
    try {
      ...
      if (createUserResponse.status === 201) {
        ...
        return NextResponse.json({ message: 'User created successfully', userId: userId }, { status: 201 });
      } else {
      ...
      return NextResponse.json({ message: 'An error occurred while trying to create user', error: errorMessage }, { status: 500 });
    }

  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred';
    return NextResponse.json({ message: 'An error occurred while obtaining admin token', error: errorMessage }, { status: 500 });
  }
}