...
async function syncUsers() {
  try {
    ...
    const existingKeycloakUsers: KeycloakUserRepresentation[] = await getUsersResponse.json();
    ...
    const keycloakUsersByEmail = new Map<string, KeycloakUserRepresentation>();
    ...
    for (const row of sheetDataRows) {
      ...
      const sheetRowData: { [key: string]: string | undefined } = {};
