...
async function extractData(client, query, limit = 1000) {
    ...
    try {
        ...
        const data = res.rows.map((row) => columns.map((col) => row[col]));
        if (data.length > 0) {
...
function getGraphClient(config) {
...
function getWorkbookTablePath(config, tableId) {
...
async function loadToExcelOnline(graphClient, config, columns, data) {
    var _a;
    ...
    try {
        table = await graphClient.api(tablePath).get();
        console.info(`Table '${config.tableName}' found.`);
    }
    catch (error) {
        const graphError = error; // Added semicolon for consistency
        if (graphError.statusCode === 404) {
            ...
            try {
                ...
                table = await graphClient.api(newTablePath).post(createTablePayload);
    ...
    try {
        ...
        const headerRowRange = await graphClient.api(headerRowRangePath).get();
        let headersExist = false;
        if (headerRowRange && headerRowRange.values && headerRowRange.values[0]) {
            const existingHeaders = headerRowRange.values[0].map((h) => String(h).trim());
            if (existingHeaders.length === columns.length && existingHeaders.every((h, i) => String(h) === columns[i])) {
                headersExist = true;
            }
        }
        if (!headersExist && columns.length > 0) {
            ...
            if (!(headerRowRange === null || headerRowRange === void 0 ? void 0 : headerRowRange.values) || headerRowRange.values.length === 0 || ((_a = headerRowRange.values[0]) === null || _a === void 0 ? void 0 : _a.every((v) => String(v) === ""))) {
    ...
    const formattedData = data.map(row => row.map(value => {
...
export async function exportDbToSharePoint() {
