...
async function getRoleDetails(accessToken, roleName) {
    const keycloakUrl = process.env.KEYCLOAK_URL;
    const keycloakRealm = process.env.KEYCLOAK_REALM;
...
async function getCompositeRoles(accessToken, roleName) {
    const keycloakUrl = process.env.KEYCLOAK_URL;
    const keycloakRealm = process.env.KEYCLOAK_REALM;
    if (!keycloakUrl || !keycloakRealm) {
        throw new Error('Missing Keycloak environment variables');
    ...
    if (response.ok) {
        ...
        console.log(`Current composite roles for "${roleName}":`, composites.map((r) => r.name));
        return composites;
    }
    else {
        const errorData = await response.text();
...
async function removeRoleFromComposite(accessToken, parentRoleName, childRoleId) {
    const keycloakUrl = process.env.KEYCLOAK_URL;
    const keycloakRealm = process.env.KEYCLOAK_REALM;
    if (!keycloakUrl || !keycloakRealm) {
        throw new Error('Missing Keycloak environment variables');
...
async function main() {
    try {
        ...
        if (currentDefaultRoles.includes('default-roles-master')) {
            ...
            realmSettings.defaultRoles = currentDefaultRoles.filter((role) => role !== 'default-roles-master');
            const updateRealmResponse = await fetch(realmSettingsUrl, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${accessToken}`,
        ...
        const isContractorInComposites = defaultRolesMasterComposites.some((role) => role.id === contractorRoleId);
