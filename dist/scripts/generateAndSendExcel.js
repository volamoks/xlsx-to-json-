...
dotenv.config();
async function getRowsFromSheet(folderId) {
    ...
    if (folderIdIndex === -1)
        throw new Error("Column 'request_position_folder_id' not found.");
    const filteredRows = allRows.slice(1).filter((row) => row[folderIdIndex] !== null && parseInt(String(row[folderIdIndex]), 10) === folderId);
...
async function generateExcelBuffer(positions) {
    // ... (this function remains the same)
    ...
    if (positions.length > 0) {
        ...
        const sanitizedPositions = positions.map(pos => {
            ...
            for (const key in pos) {
                sanitized[key] = (_a = pos[key]) !== null && _a !== void 0 ? _a : '';
            }
            return sanitized;
        });
...
export async function generateAndUploadToOneDrive(folderId) {
    try {
